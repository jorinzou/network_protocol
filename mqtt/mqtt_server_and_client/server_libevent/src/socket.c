...
void delete_sockfd_record(int sockfd)
{
	...
	if (src_record) {
        ...
        char ip_str[16] = {0};
        in_addr_to_str(&(src_record->client_addr),ip_str,sizeof(ip_str)-1);
        DEBUG_INFO("client %s close",ip_str);
		pre_record->next = src_record->next;
        event_del(&(src_record->ev));
...
int accept_connect(int sockfd, short event, void *argc)
{
	int tr = 1;
    ...
    char ip_str[16] = {0};
    in_addr_to_str(&(new_sockfd->client_addr),ip_str,sizeof(ip_str)-1);
    DEBUG_INFO("client %s connect",ip_str);
	...
	event_set(&(new_sockfd->ev),new_sockfd->sockfd,EV_READ | EV_PERSIST, do_socket, NULL);
    /*将event添加到libevent侦听的事件集中*/
    if (event_add(&(new_sockfd->ev), 0) < 0){
        DEBUG_INFO("can not add event into libevent");
        close(new_sockfd->sockfd);
        free(new_sockfd);
        return -1;
    }	  	    
...
int socket_init(int port)
{
    ...
    if (NULL == socket_record_head) { 
        ...
        event_set(&(sockfd_record->ev), sockfd_record->sockfd, EV_READ|EV_PERSIST, accept_connect, NULL); 
		/*将event添加到libevent侦听的事件集中*/
        if (event_add(&(sockfd_record->ev), 0) < 0){
            DEBUG_INFO("can not add event into libevent");
            close(sockfd_record->sockfd);
            free(sockfd_record);
            socket_record_head = NULL;
            return -1;
        }	   
    }
    return 0;
